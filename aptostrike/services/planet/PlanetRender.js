import { getRandGenForHash } from "./hash";
...
class PlanetRender {
    ...
    constructor(planetHash, canvasRef) {
        ...
        this.structs = {};
        this.slots = {};
        this.randForPlanetHash = getRandGenForHash(planetHash);
        this.planetParams = {
            vWaterLevel: 0,
            vRivers: 0,
            vTemperature: 0,
    ...
    #doGen(structID) {
        ...
        this.structs[structID]?.slots.forEach((slot) => {
            ...
            result[slot] =
                availableSlots[Math.floor(this.randForPlanetHash() * availableSlots.length)];
        });
        return result;
    };
    ...
    #doDisplay() {
        ...
        this.planetParams.vAngle = 0.6 * this.randForPlanetHash();
        this.planetParams.vRotspeed =
            (0.005 + this.randForPlanetHash() * 0.01) *
            (this.randForPlanetHash() < 0.3 ? -1 : 1) *
            eval(this.#doExpand(result.struct.vals["rotspeedMult"], result));
            this.planetParams.vLight = 4 * this.randForPlanetHash();
            this.planetParams.vZLight = 0.2 + this.randForPlanetHash();
            this.planetParams.vModValue = 17 + Math.ceil(this.randForPlanetHash() * 20);
            this.planetParams.vNoiseOffset = [Math.ceil(this.randForPlanetHash() * 100), Math.ceil(this.randForPlanetHash() * 100)];
            this.planetParams.vNoiseScale = [6 + Math.ceil(this.randForPlanetHash() * 8), 5 + Math.ceil(this.randForPlanetHash() * 6)];
        let sc = 80 + Math.ceil(this.randForPlanetHash() * 220);
        this.planetParams.vNoiseScale2 = [sc, sc];
        sc = 20 + Math.ceil(this.randForPlanetHash() * 80);
        this.planetParams.vNoiseScale3 = [sc, sc];
        this.planetParams.vCloudNoise = [4 + Math.ceil(this.randForPlanetHash() * 9), 20 + Math.ceil(this.randForPlanetHash() * 20)];
    };
